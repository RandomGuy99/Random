...
import com.verizon.delphi.model.ModCardInfo;
import com.verizon.delphi.model.ModuleInfo;
import com.verizon.delphi.model.ModulePortInfo;
...
import com.verizon.delphi.model.PortCardInfo;
import com.verizon.delphi.model.PortInfo;
...
public class FlowServiceImpl  implements FlowService {
	...
	private TopologyResponse populateTopologyData(NeatInfo neatInfo) {
		...
		ModCardInfo moduleCardInfo = new ModCardInfo();
		PortCardInfo portCardInfo = new PortCardInfo();
		ModuleInfo moduleInfo = new ModuleInfo();
		PortInfo portInfo = new PortInfo();
		ModulePortInfo mPortInfo = new ModulePortInfo();
		SlotInfo slotInfo = new SlotInfo();
		ShelfInfo shelfInfo = new ShelfInfo();
		EquipNodeInfo equinodeInfo = new EquipNodeInfo();
		EquipmentView equipmentView = new EquipmentView();

		List<NodeInfo> nodeInfoList = new ArrayList<NodeInfo>();
		List<EquipNodeInfo> equiNodeInfoList = new ArrayList<EquipNodeInfo>();
		List<ShelfInfo> shelfInfoList = new ArrayList<ShelfInfo>();
		List<SlotInfo> slotInfoList = new ArrayList<SlotInfo>();
		List<ModulePortInfo> modulePortInfoList = new ArrayList<ModulePortInfo>();
		List<ModuleInfo> moduleInfoList = new ArrayList<ModuleInfo>();
		List<PortInfo> portInfoList = new ArrayList<PortInfo>();
		List<ModCardInfo> moduleCardInfoList = new ArrayList<ModCardInfo>();
		List<PortCardInfo> portCardInfoList = new ArrayList<PortCardInfo>();
		...
		EquipNodeInfo childrennode = new EquipNodeInfo();
		NodeAttribute nodeAttribute = null;
		String nodeToken = "";
		...
		for (NodeAttribute nodeAttr : nodeAttributeList) {
			...
			equiNodeInfoList.add(childrennode);
		...
		childrennode.setId(nodeToken);
		childrennode.set_class("SHELF");
		childrennode.setLabel("Shelf123");
		childrennode.setChildren(shelfInfoList);
		
		equipmentView.setId("123");
		equipmentView.setLabel("EquipLabel");
		equinodeInfo.setId("234");
		equinodeInfo.setLabel("EquipNodeInfo");
		shelfInfo.setId("345");
		shelfInfo.setLabel("ShelfInfo");
		slotInfo.setId("456");
		slotInfo.setLabel("SlotInfo");
		moduleInfo.setId("111");
		moduleInfo.setLabel("Module Info");
		moduleInfo.set_class("MODULE");
		portInfo.setId("222");
		portInfo.setLabel("Port Info");
		portInfo.set_class("PORT");
		moduleCardInfo.setId("3333");
		moduleCardInfo.setLabel("Module CardInfo");
		moduleCardInfo.set_class("CARD");
		portCardInfo.setId("4444");
		portCardInfo.setLabel("Port Card Info");
		portCardInfo.set_class("CARD");
	
		slotInfo.setChildren(modulePortInfoList);
		mPortInfo.setMchildren(moduleCardInfoList);
		mPortInfo.setPchildren(portCardInfoList);
		moduleCardInfo.setChildren(moduleInfoList);
		portCardInfo.setChildren(portInfoList);
		shelfInfo.setChildren(slotInfoList);
		equinodeInfo.setChildren(shelfInfoList);
		equipmentView.setChildren(equiNodeInfoList);
		attributes.setNodeAttribute(nodeAttrList);

		topology.setAttributes(attributes);
		topology.setSignalFlow(signalFlow);
		topology.setEquipmentView(equipmentView);
